...
// 获取评论列表（不包括已删除的）
$comments = getDineInCommentsByReviewId($review_id);

$pageTitle = "聚餐评价详情 - " . htmlspecialchars($review['restaurant_name']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($content)) {
        ...
        $result = addDineInComment($review_id, $content);
        if ($result) {
            $success = '评论添加成功！';
            // 重新获取评论列表
            $comments = getDineInCommentsByReviewId($review_id);
